...
| 指标 | `macd_cross`、`ma_cross`、`boll_trend` 三类 DK 趋势近似 |
| 信号 | 量能确认、三模式共振、`min_run_len` 防抖、信号质量初步评分 |
| 回测 | T+1 次日开盘执行、涨停买入顺延、停牌近似、交易成本、固定/移动/ATR 止损 |
| 风控 | 基于单笔风险的仓位、指数极端下跌入场过滤、止损后再入场 |
| 评估 | 批量回测、WFO、DSR、Bootstrap Sharpe CI、置换检验、HTML 报告 |
